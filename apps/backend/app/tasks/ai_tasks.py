<<<<<<< Updated upstream
# Tasks for AI processing (vectorization, graph updates)
=======
...
def process_todos_from_extraction(extraction: dict, journal_entry_id: int, user_id: str):
    ...
    Args:
        ...
        user_id: User ID owning the entry
    """
    logger.info(f"Starting process_todos_from_extraction for journal_entry_id: {journal_entry_id}")
    extraction_result = ExtractionResult(**extraction)
    
    if not extraction_result.todos:
        logger.info("No todos to process")
        return
    ...
    try:
        ...
        for todo in extraction_result.todos:
            # Map priority from extraction to TodoCreate Priority enum
            priority_map = {
                "must_do": Priority.HIGH,
                "high": Priority.HIGH,
                "medium": Priority.MEDIUM,
                "normal": Priority.MEDIUM,
                "low": Priority.LOW
            }
            priority = priority_map.get(todo.priority.lower(), Priority.MEDIUM)
            ...
            todo_create = TodoCreate(
                task=todo.task,
                priority=priority.value,  # Convert enum to string value
                due_date=due_date,
            ...
            logger.info(f"Created todo: {created_todo.id} - {created_todo.task}")
...
def process_calendar_events_from_extraction(
    ...
    print(f"\n{'='*60}")
    print(f"CALENDAR TASK DEBUG - Journal Entry ID: {journal_entry_id}")
    print(f"{'='*60}")
    ...
    print(f"Total events in extraction: {len(extraction_result.events)}")
    ...
    print(f"Events to sync (should_sync_calendar=True): {len(events_to_sync)}")
    for idx, e in enumerate(events_to_sync, 1):
        print(f"  Event {idx}: {e.title} - {e.datetime}")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        for event in events_to_sync:
            ...
            print(f"\nAttempting to create calendar event: {event.title}")
            print(f"  Start: {start}")
            print(f"  End: {end}")
            ...
            print(f"✓ Successfully created event! Event ID: {created_event.get('id')}")
            logger.info(f"Created calendar event: {created_event.get('id')} - {event.title}")
    
    except Exception as e:
        print(f"\n✗ ERROR in calendar task: {e}")
